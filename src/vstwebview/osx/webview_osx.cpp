...
public:
    WebviewOSX(bool debug, Steinberg::IPlugFrame *plug_frame,
               id parentView, WebviewCreatedCallback created)
    {
        ...
        if ( cls == NULL ) {
            cls =
            objc_allocateClassPair((Class) "NSResponder"_cls, "WebviewDelegate", 0);
            class_addMethod(cls, "userContentController:didReceiveScriptMessage:"_sel,
                            (IMP)(+[](id self, SEL, id, id msg) {
                              auto w =
                                  (WebviewOSX *)objc_getAssociatedObject(
                                      self, "webview");
                              assert(w);
       ...
       ((void (*)(id, SEL, CGRect, id))objc_msgSend)(
                webview_, "initWithFrame:configuration:"_sel,
    ...
    std::string ContentRootURI() const override {
        ...
        CFURLRef url = CFURLCreateWithString(NULL,
                                             CFStringCreateWithCString(NULL, path.c_str(),
                                             kCFStringEncodingUTF8), NULL);
        url = CFURLCreateCopyDeletingLastPathComponent(NULL, url);
        url = CFURLCreateCopyDeletingLastPathComponent(NULL, url);
        url = CFURLCreateCopyAppendingPathComponent(NULL, url,
                                                    CFStringCreateWithCString(NULL, resources.c_str(),
                                    ...
                                    WebviewCreatedCallback created_cb)
{
    ...
    auto webview    = std::make_unique<WebviewOSX>(debug,
                                                plug_frame,
                                                parentView,
                                                created_cb
                                                );
